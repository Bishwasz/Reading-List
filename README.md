#  Reads

## Systems / ML at Scale
- Pipeline Parallel Training (GEMM): https://siboehm.com/articles/22/pipeline-parallel-training
- JAX Scaling Book: https://jax-ml.github.io/scaling-book/
- Salykova (Systems / ML): https://salykova.github.io/
- Siboehm (About / Writing): https://siboehm.com/about/
- LLaMA 3 Web Notes: https://saurabhalone.com/blogs/llama3/web

## Reinforcement Learning
- TRPO Explained (Part 1): https://jonathan-hui.medium.com/rl-trust-region-policy-optimization-trpo-explained-a6ee04eeeee9
- TRPO Explained (Part 2): https://jonathan-hui.medium.com/rl-trust-region-policy-optimization-trpo-part-2-f51e3b2e373a

## C++
- Smart Pointers & Move Semantics: https://www.learncpp.com/cpp-tutorial/introduction-to-smart-pointers-move-semantics/

## Signal Processing
- Fourier Transform (Brian McFee): https://brianmcfee.net/dstbook-site/content/ch05-fourier/intro.html

## Interviews / Performance
- ML Interviews (NLP): https://huyenchip.com/ml-interviews-book/contents/8.2.1-natural-language-processing.html
- Huỳnh Chip’s Blog: https://huyenchip.com/blog/
- CPU Cache Sharing: https://en.algorithmica.org/hpc/cpu-cache/sharing/
- CUDA Optimization Tutorial: https://cnugteren.github.io/tutorial/pages/page12.html

## vLLM
- vLLM Blog Post: https://blog.vllm.ai/2023/06/20/vllm.html
- vLLM Paper (arXiv): https://arxiv.org/pdf/2309.06180

## CPU Architecture
- Load Speculation (Calder et al.): https://cseweb.ucsd.edu/~calder/papers/JILP-00-LoadSpec.pdf
- SIMD Data Movement: https://en.algorithmica.org/hpc/simd/moving/

## Statistics
- General Statistics definition: https://thestatsmap.com/sequential-statistics
- KL Approximation (Joschu): http://joschu.net/blog/kl-approx.html

## GPU Architecture
- GPU Architecture Report (Arora): https://cseweb.ucsd.edu/~marora/files/papers/REReport_ManishArora.pdf
- Warp-Level Parallelism (MICRO 2016): https://people.ece.ubc.ca/aamodt/publications/papers/eltantawy.micro2016.pdf
- Dynamic Parallelism (HPCA 2013): https://lph.ece.utexas.edu/merez/uploads/MattanErez/hpca2013_dpe.pdf
- Berkeley GPU Research Report: https://www2.eecs.berkeley.edu/Pubs/TechRpts/2016/EECS-2016-143.pdf
- GPU Microarchitecture (IEEE): https://ieeexplore.ieee.org/stamp/stamp.jsp?tp=&arnumber=4523358

## Compilers
- Compiler Construction Book (Thain): https://www3.nd.edu/~dthain/compilerbook/
- Gaussian Elimination (CP-Algorithms): https://cp-algorithms.com/linear_algebra/determinant-gauss.html
